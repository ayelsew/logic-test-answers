  ...
  */
  ...
  function question1() {
    ...
    let prepoArray, prepositionsFromB;

    prepoArray = filterPrepositionsFrom(wordsB);

    prepositionsFromB = getValuesForRules(prepoArray, rulePreposition);
    
    console.log(`Resposta: Existem ${prepositionsFromB.length} preposições no Texto B.`);
  }
  ...
  */
  ...
  function question2() {
    ...
    let verbsArray, verbsFromB, firstPersonVerbs;
    
    verbsArray = filterVerbsFrom(wordsB);

    verbsFromB = getValuesForRules(verbsArray, ruleVerb);
    console.log(`Resposta: Há ${verbsFromB.length} verbos no Texto B.`);  

    firstPersonVerbs = getValuesForRules(verbsFromB, ruleFirstPersonVerb);
    console.log(`Resposta: Há ${firstPersonVerbs.length} verbos em primeira pessoa no Texto B.`);  
  ...
  */
  ...
  function question3() {
    ...
    let sortedArray, vocabularyFromB

    sortedArray = getValuesForRules(wordsB, ruleAlphabeticalOrder);

    vocabularyFromB = removeDuplicates(sortedArray)
    console.log(`Resposta: lista de vocabulário do Texto B ${vocabularyFromB}`)
  ...
  */
  ...
  function question4() {
    ...
    let letters, numbers, beautifulNumbers;
    
    mapLetterToNumber(map)

    letters = splitLettersFrom(wordsB)

    numbers = parseLetterToNumbers(letters, map)
    
    beautifulNumbers = getBeautifulNumbers(numbers)

    removeDuplicates(beautifulNumbers)
    console.log(`Resposta: No Texto B, há ${beautifulNumbers.length} bonitos distintos (atenção!). ${beautifulNumbers}`)
  ...
  */
  ...
  function getValueOfLetter(letter) { return map.get(letter) }

  function splitLettersFrom(arr) { return arr.map(word => word.split('')) }

  function reduceArray(arr) { return arr.reduce((a, b) => a.concat(b)) }

  function mapLetterToNumber(map) { ruleAlphabeticalOrder.map((letter, i) => map.set(letter, i)) }

  function removeDuplicates(arr) { return [... new Set(arr)] }

  function splitText(text) { return text.split(' ') }

  function filterPrepositionsFrom(arr) { return arr.filter(word => word.length === 3) }

  function filterVerbsFrom(arr) { return arr.filter(word => word.length >= 7) }

  function getValuesForRules(arr, rules) {
    let array = rules.map(rule => {
      return arr.filter(word => {
        if (rules === rulePreposition && word.slice(-1).includes(rule) && !word.includes('b')) {
          return word
        } else if (rules === ruleVerb && word.slice(-1).includes(rule)) {
          return word
        } else if ((rules === ruleFirstPersonVerb || rules === ruleAlphabeticalOrder) && word.slice(0,1).includes(rule)) {
          return word
        }
      });
    });
  ...
  function getBeautifulNumbers(arr) {
    return arr.filter(number => {
      if (number >= 787808 && number % 3 === 0) {
        return number
      }
    });
  }

  function parseLetterToNumbers(arr, map) {
    let numbers = []
    arr.map(value => {
      let number = []
      value.forEach(e => {
        number.push(e.replace(new RegExp(e, 'g'), getValueOfLetter(e)))
      })
      numbers.push(reduceArray((number)))
    })
    return numbers
  }